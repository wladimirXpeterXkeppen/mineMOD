...
#minemod-mp3 input[type=range]{width:100%;accent-color:#00cfff;cursor:pointer}.mp3-controls{display:flex;justify-content:center;align-items:center;gap:7px;margin:9px 0}.mp3-controls .main{width:44px;height:38px;border-radius:11px;font-size:17px}
...
.mp3-bottom{display:flex;gap:7px;margin-top:10px;align-items:center}.mp3-file{width:1px;height:1px;opacity:0;position:absolute;pointer-events:none}.mp3-add{display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box}
.mp3-drop{margin-top:8px;padding:9px;text-align:center;border:1px dashed rgba(0,207,255,.28);border-radius:9px;color:#7f98aa;font-size:11px}.mp3-drop.drag{border-color:#00eaff;background:rgba(0,234,255,.08);color:#fff}
`;
  ...
  function build(){
    ...
    root.innerHTML=`
      ...
      <div class="mp3-body">
        ...
        <div class="mp3-bottom">
          <label class="mp3-add" for="minemod-mp3-file">＋ Add MP3</label>
    ...
    root.querySelector(".mp3-add").style.cssText='color:#fff;background:rgba(0,207,255,.08);border:1px solid rgba(0,207,255,.22);border-radius:8px;cursor:pointer;padding:6px 9px;font:inherit';
    document.body.appendChild(root);

    root.addEventListener("click",e=>{
      const el=e.target.closest("[data-a]");if(!el)return;
      const a=el.dataset.a;
      if(a==="close")toggle(false); else if(a==="clear")clear(); else if(a==="play")playPause(); else if(a==="prev")prev(); else if(a==="next")next(); else if(a==="shuffle")toggleShuffle(); else if(a==="repeat")toggleRepeat();
    });
  ...
  function destroy(){audio?.pause();objectUrls.forEach(URL.revokeObjectURL);root?.remove();style?.remove();delete window.__MineModMP3;delete window.__MineModMP3Integration}

  build();makeAudio();render();
  window.__MineModMP3={open,close:()=>toggle(false),toggle,addFiles,clear,destroy,state};

  // Integrate into the existing Sonnet radial menu without replacing it.
  const register=()=>{
