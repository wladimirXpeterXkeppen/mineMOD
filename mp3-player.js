...
(()=>{
  ...
  const state={files:[],index:-1,shuffle:false,repeat:"off",volume:.8,open:false};
  let audio=null,root=null,style=null,urls=[];

  const css=`
#minemod-mp3{position:fixed;width:min(400px,92vw);margin:0;right:20px;bottom:20px;padding:0;z-index:2147483647;color:#fff;background:linear-gradient(145deg,rgba(14,23,38,.98),rgba(3,8,17,.98));border:1px solid rgba(0,234,255,.5);border-radius:16px;box-shadow:0 0 40px rgba(0,217,255,.25),inset 0 1px rgba(255,255,255,.08);font:13px Inter,"Segoe UI",Arial,sans-serif;overflow:hidden}
#minemod-mp3:not([open]){display:none}
#minemod-mp3::backdrop{background:transparent;pointer-events:none}
...
#minemod-mp3 button{color:#fff;background:rgba(0,207,255,.08);border:1px solid rgba(0,207,255,.25);border-radius:8px;cursor:pointer;padding:7px 10px;font:inherit}
#minemod-mp3 button:hover{background:rgba(0,207,255,.2);border-color:#00eaff}
...
#minemod-mp3 .mp3-options button{font-size:11px;padding:5px 7px}.mp3-options .on{color:#00eaff;border-color:#00eaff;background:rgba(0,234,255,.14)}
#minemod-mp3 .mp3-volume{display:flex;align-items:center;gap:8px;flex:1;color:#91a6b8}.mp3-volume input{flex:1}
#minemod-mp3 .mp3-list{max-height:180px;overflow:auto;border-top:1px solid rgba(255,255,255,.07);margin:0 -14px;padding:5px 8px}
#minemod-mp3 .mp3-item{display:flex;align-items:center;gap:8px;padding:8px 7px;border-radius:8px;cursor:pointer;color:#c9d5df}.mp3-item:hover{background:rgba(0,207,255,.08)}.mp3-item.current{background:rgba(0,207,255,.14);color:#fff}.mp3-num{width:20px;color:#668094;text-align:center}.mp3-item-name{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
#minemod-mp3 .mp3-bottom{display:flex;gap:7px;margin-top:10px;align-items:center}
#minemod-mp3 .mp3-file{position:absolute;width:1px;height:1px;opacity:0;overflow:hidden}
#minemod-mp3 .mp3-add{display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;color:#fff;background:rgba(0,207,255,.08);border:1px solid rgba(0,207,255,.25);border-radius:8px;cursor:pointer;padding:7px 10px}
#minemod-mp3 .mp3-drop{margin-top:8px;padding:9px;text-align:center;border:1px dashed rgba(0,207,255,.35);border-radius:9px;color:#7f98aa;font-size:11px;cursor:pointer}.mp3-drop.drag{border-color:#00eaff;background:rgba(0,234,255,.08);color:#fff}
`;

  const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

  function build(){
    style=document.createElement("style");style.id="minemod-mp3-style";style.textContent=css;document.head.appendChild(style);
    root=document.createElement("dialog");root.id="minemod-mp3";
    root.innerHTML=`
      <div class="mp3-head"><div class="mp3-title">♫ MP3 Player</div><button type="button" data-a="close">×</button></div>
      <div class="mp3-body">
        ...
        <div class="mp3-controls"><button type="button" data-a="prev">⏮</button><button type="button" class="main" data-a="play">▶</button><button type="button" data-a="next">⏭</button></div>
        <div class="mp3-options"><button type="button" data-a="shuffle">Shuffle</button><button type="button" data-a="repeat">Repeat: Off</button><div class="mp3-volume">🔊 <input class="volume" type="range" min="0" max="1" step=".01" value=".8"></div></div>
        <div class="mp3-list"></div>
        <div class="mp3-bottom"><label class="mp3-add" for="minemod-mp3-file">＋ Add MP3</label><input id="minemod-mp3-file" class="mp3-file" type="file" accept=".mp3,audio/mpeg,audio/*" multiple><button type="button" data-a="clear">Clear</button></div>
  ...
  function addFiles(files){
    ...
    if(!accepted.length){console.warn("[mineMOD MP3] MP3 file not selected");return}
    accepted.forEach(f=>{const url=URL.createObjectURL(f);urls.push(url);state.files.push({name:f.name,url})});
    if(state.index<0)select(0,false);render();
  }
  function select(i,autoplay=true){if(!state.files[i])return;state.index=i;audio.src=state.files[i].url;audio.load();render();if(autoplay)audio.play().catch(err=>console.warn("[mineMOD MP3] playback blocked",err))}
  function playPause(){if(!state.files.length){console.warn("[mineMOD MP3] Add an MP3 first");return}if(state.index<0)return select(0,true);if(audio.paused)audio.play().catch(err=>console.warn("[mineMOD MP3] playback blocked",err));else audio.pause();render()}
  ...
  function clear(){state.files=[];state.index=-1;urls.forEach(URL.revokeObjectURL);urls=[];if(audio){audio.pause();audio.removeAttribute("src");audio.load()}render()}
  const fmt=s=>Number.isFinite(s)?`${Math.floor(s/60)}:${String(Math.floor(s%60)).padStart(2,"0")}`:"0:00";
  function renderProgress(){if(!root||!audio)return;const p=root.querySelector(".mp3-progress");p.value=audio.duration?audio.currentTime/audio.duration*100:0;root.querySelector(".cur").textContent=fmt(audio.currentTime||0);root.querySelector(".dur").textContent=fmt(audio.duration||0);root.querySelector("[data-a=play]").textContent=audio.paused?"▶":"⏸"}
  function renderList(){root.querySelector(".mp3-list").innerHTML=state.files.map((f,i)=>`<div class="mp3-item ${i===state.index?"current":""}" data-index="${i}"><span class="mp3-num">${i+1}</span><span class="mp3-item-name">${esc(f.name)}</span></div>`).join("");root.querySelectorAll(".mp3-item").forEach(x=>x.onclick=()=>select(+x.dataset.index,true))}
  ...
  function close(){state.open=false;if(root.open)root.close()}
  function toggle(v){v===undefined?(state.open?close():open()):(v?open():close())}
  function destroy(){close();audio?.pause();urls.forEach(URL.revokeObjectURL);root?.remove();style?.remove();delete window.__MineModMP3;delete window.__MineModMP3Integration}

  build();makeAudio();render();
  window.__MineModMP3={open,close,toggle,addFiles,clear,destroy,state};
  ...
  console.log("[mineMOD] MP3 Player v2 / dialog top-layer / Add MP3 ready / Right Shift");
})();
