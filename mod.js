...
(() => {
    ...
    function createUI(){const root=document.createElement("div");root.id="galaxy-ui";root.innerHTML=`<div id="galaxy-overlay"></div><div id="galaxy-wheel"><div id="galaxy-ring"></div><div id="galaxy-buttons"></div><div id="galaxy-center"><span id="galaxy-center-text">Galaxy</span></div></div>`;document.body.appendChild(root);Galaxy.ui=root;Galaxy.wheel=root.querySelector("#galaxy-wheel");Galaxy.center=root.querySelector("#galaxy-center");Galaxy.center.addEventListener("click",()=>Galaxy.current==="main"?close():showMain());applyTheme()}
    ...
    function createRadialButtons(items,back=false){
        clearButtons();
        const c=Galaxy.ui.querySelector("#galaxy-buttons"),list=back?[...items,"← 戻る"]:items,r=205,step=Math.PI*2/list.length;
        list.forEach((name,i)=>{
            ...
            b.addEventListener("click",e=>{
                e.stopPropagation();
                if(name==="← 戻る"){showMain();resumeRotation();return}
                if(Galaxy.current==="main"){showCategory(name);resumeRotation();return}
                if(name==="Theme"){showThemes();resumeRotation();return}
                toggleModule(b,name);
            });
    ...
    function showThemes(){
        Galaxy.current="theme";clearButtons();
        const keys=Object.keys(themes),c=Galaxy.ui.querySelector("#galaxy-buttons"),r=205,step=Math.PI*2/keys.length;
        keys.forEach((key,i)=>{
            const name=themes[key].name,b=document.createElement("button");b.type="button";b.className="galaxy-button";b.dataset.theme=key;
            const a=-Math.PI/2+step*i;b.style.left=`calc(50% + ${Math.cos(a)*r}px)`;b.style.top=`calc(50% + ${Math.sin(a)*r}px)`;b.innerHTML=`<span class="galaxy-label">${esc(name)}</span>`;b.classList.toggle("active",key===Galaxy.theme);
            b.onclick=e=>{e.stopPropagation();Galaxy.theme=key;applyTheme();showThemes();resumeRotation()};
            b.onmouseenter=()=>{Galaxy.paused=true};b.onmouseleave=()=>{Galaxy.paused=false};c.appendChild(b);Galaxy.buttons.push(b)
        });
    }
