...
(() => {
    ...
    const Galaxy={open:false,angle:0,speed:.10,paused:false,raf:0,ui:null,wheel:null,center:null,centerText:null,buttons:[],current:"main",theme:localStorage.getItem("galaxy_theme")||"gaming"};
    ...
    function createUI(){
        ...
        root.innerHTML=`<div id="galaxy-overlay"></div><div id="galaxy-wheel"><div id="galaxy-ring"></div><div id="galaxy-buttons"></div><div id="galaxy-center"><div id="galaxy-center-logo" aria-label="Sonnet"><span></span><i></i><b></b></div><span id="galaxy-center-text"></span></div></div>`;
        document.body.appendChild(root);Galaxy.ui=root;Galaxy.wheel=root.querySelector("#galaxy-wheel");Galaxy.center=root.querySelector("#galaxy-center");Galaxy.centerText=root.querySelector("#galaxy-center-text");
    ...
    function createRadialButtons(items,back=false){
        ...
        list.forEach((name,i)=>{const b=document.createElement("button");b.type="button";b.className="galaxy-button";b.dataset.module=name;b.dataset.enabled="false";if(name==="← 戻る")b.dataset.returnButton="true";
            ...
            b.addEventListener("click",e=>{e.stopPropagation();if(name==="← 戻る"){showMain();return}if(Galaxy.current==="main"){showCategory(name);return}if(name==="Theme"){showThemes();return}toggleModule(b,name)});
    ...
    function showCategory(category){Galaxy.current=category;setCenter(category,false);createRadialButtons(MODULES[category]||[],true);Galaxy.angle=0;Galaxy.wheel.style.setProperty("--rotation","0deg");resumeRotation()}
    function showThemes(){
        Galaxy.current="theme";setCenter("Theme",false);clearButtons();Galaxy.angle=0;Galaxy.wheel.style.setProperty("--rotation","0deg");
        const keys=Object.keys(themes),c=Galaxy.ui.querySelector("#galaxy-buttons"),r=205,step=Math.PI*2/keys.length;
        keys.forEach((key,i)=>{const name=themes[key].name,b=document.createElement("button");b.type="button";b.className="galaxy-button";b.dataset.theme=key;const a=-Math.PI/2+step*i;
            b.style.left=`calc(50% + ${Math.cos(a)*r}px)`;b.style.top=`calc(50% + ${Math.sin(a)*r}px)`;b.innerHTML=`<span class="galaxy-label">${esc(name)}</span>`;b.classList.toggle("active",key===Galaxy.theme);
            b.onclick=e=>{e.stopPropagation();Galaxy.theme=key;applyTheme();showThemes()};b.onmouseenter=()=>{Galaxy.paused=true};b.onmouseleave=()=>{Galaxy.paused=false};c.appendChild(b);Galaxy.buttons.push(b);
        });
    }
    ...
    function animate(){
        if(Galaxy.open&&Galaxy.current==="main"&&!Galaxy.paused)Galaxy.angle+=Galaxy.speed;
        const rotating=Galaxy.current==="main";
        Galaxy.wheel.style.setProperty("--rotation",`${rotating?Galaxy.angle:0}deg`);
        Galaxy.buttons.forEach(b=>{const l=b.querySelector(".galaxy-label");if(l)l.style.transform=`translate(-50%,-50%) rotate(${rotating?-Galaxy.angle:0}deg)`});
    ...
    function init(){createUI();showMain();document.addEventListener("keydown",key,true);Galaxy.raf=requestAnimationFrame(animate);window.__GalaxyUI={open,close,toggle,showMain,showCategory,showThemes,destroy,setTheme(k){if(themes[k]){Galaxy.theme=k;applyTheme()}},themes,state:Galaxy};console.log("%c[Sonnet UI] Loaded%c 右Altで開閉 / Themeでテーマ変更","color:#6fdfff;font-weight:bold","color:inherit")}
