...
(() => {
    ...
    function createRadialButtons(items,back=false){
        ...
        list.forEach((name,i)=>{const b=document.createElement("button");b.type="button";b.className="galaxy-button";b.dataset.module=name;b.dataset.enabled="false";const a=-Math.PI/2+step*i;b.style.left=`calc(50% + ${Math.cos(a)*r}px)`;b.style.top=`calc(50% + ${Math.sin(a)*r}px)`;b.innerHTML=`<span class="galaxy-label">${esc(name)}</span>`;if(name==="← 戻る")b.dataset.returnButton="true";b.addEventListener("mouseenter",()=>{if(!b.dataset.returnButton){Galaxy.paused=true;Galaxy.wheel.classList.add("galaxy-wheel-hover")}});b.addEventListener("mouseleave",()=>{Galaxy.paused=false;Galaxy.wheel.classList.remove("galaxy-wheel-hover")});b.addEventListener("click",e=>{e.stopPropagation();if(name==="← 戻る"){showMain();resumeRotation();return}if(Galaxy.current==="main")return showCategory(name);if(name==="Theme")return showThemes();toggleModule(b,name)});c.appendChild(b);Galaxy.buttons.push(b)})
    }
