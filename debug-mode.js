...
const st=document.createElement("style");st.id="minemod-debug-style";st.textContent=`#minemod-debug{position:fixed;left:8px;top:8px;z-index:2147483647;display:none;color:#fff;background:rgba(0,0,0,.68);padding:8px 10px;font:12px/1.35 monospace;white-space:pre;pointer-events:none;text-shadow:1px 1px #000;max-width:60vw;backdrop-filter:blur(2px)}#minemod-debug .title{font-weight:bold}`;document.head.appendChild(st);
function game(){const c=document.querySelector("canvas");let gl=null;try{gl=c?.getContext?.("webgl2",{preserveDrawingBuffer:false})||c?.getContext?.("webgl",{preserveDrawingBuffer:false})}catch{}return{canvas:c,gl}}
...
function render(){const{canvas,gl}=game();const mem=performance.memory;const w=innerWidth,h=innerHeight,dpr=devicePixelRatio||1;let renderer="Unavailable",vendor="Unavailable";if(gl){try{vendor=gl.getParameter(gl.VENDOR)||vendor;renderer=gl.getParameter(gl.RENDERER)||renderer}catch{}}const now=new Date();el.textContent=["mineMOD Debug",`FPS: ${state.fps.toFixed(0)}  (${state.frameMs.toFixed(1)} ms)`,`Window: ${w} × ${h}  DPR: ${dpr.toFixed(2)}`,`Canvas: ${canvas?`${canvas.width} × ${canvas.height}`:"none"}`,`WebGL: ${gl?"active":"not detected"}`,`Renderer: ${renderer}`,`Vendor: ${vendor}`,`JS Heap: ${mem?`${(mem.usedJSHeapSize/1048576).toFixed(1)} / ${(mem.jsHeapSizeLimit/1048576).toFixed(0)} MB`:"unavailable"}`,`Online: ${navigator.onLine?"yes":"no"}`,`Time: ${now.toLocaleTimeString()}`,"","Debug toggle: F8","/ = Screen Color preset cycle"].join("\n")}
function toggle(){state.open=!state.open;el.style.display=state.open?"block":"none";if(state.open)render()}
function key(e){if(e.key==="F8"){e.preventDefault();toggle()}}
